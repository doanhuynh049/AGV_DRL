...
# Import various classes and functions from other files
...
class DrlAgent(Node):
    def __init__(self, algorithm, training, load_session="", load_episode=0, train_stage=util.test_stage):
        ...
        # Store various parameters for this agent such as the algorithm it uses, whether it is being trained or not, etc.
        ...
        # Quit if there is no test agent specified and this is not a training run
        ...
        # Get the simulation speed for this agent
        self.sim_speed = util.get_simulation_speed(self.train_stage)
        
        # Print out some information about the agent
        print(f"{'training' if (self.is_training) else 'testing' } on stage: {util.test_stage}")

        # Create a model according to the specified algorithm
        ...
        # Create a replay buffer for this agent
        self.replay_buffer = ReplayBuffer(self.model.buffer_size)
        
        # Create a graph to record data for this agent
        self.graph = Graph()
        ...
        # Create a storage manager for this agent
        self.sm = StorageManager(self.algorithm, self.train_stage, self.load_session, self.episode, self.device)

        # If we're loading a session, load the corresponding model, weights, etc.
        if self.load_session:
            ...
            # If we're not loading a session, create a new session directory and store the model
            self.sm.new_session_dir()
            self.sm.store_model(self.model)

        # Set the session directory and create a logger for this agent
        ...
        # Create a visualizer for this agent if enabled and attach it to the model
        if ENABLE_VISUAL:
        ...
        # Create various communication clients to interact with the environment
        ...
        # Begin running the process for this agent
        self.process()
